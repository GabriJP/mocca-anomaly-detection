        ...
        for idx, (data, _) in enumerate(tqdm(train_loader, total=len(train_loader), leave=False)):
            ...
            if idx != 0 and idx % ((len(train_loader) // log_frequency) + 1) == 0:
                ...
                for _, k in enumerate(d_from_c.keys()):
                    logger.info(f"[{k}] -- Radius: {R[k]:.4f} - " f"Dist from sphere centr: {d_from_c[k] / idx:.4f}")
...
def test(
    ...
    with torch.no_grad():
        for idx, (data, labels) in enumerate(
            tqdm(test_loader, total=len(test_loader), desc=f"Testing class: {normal_class}", leave=False)
        ):
            if debug and idx == 5:
                break
            ...
            if is_texture:
                ...
                scores = torch.stack(
                    [
                        get_scores(zipped=net(batch), c=c, R=R, device=device, boundary=boundary, is_texture=is_texture)
            ...
            idx_label_score += list(zip(labels.cpu().data.numpy().tolist(), scores.cpu().data.numpy().tolist()))
