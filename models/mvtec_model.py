...
from .mvtec_base_model import BaseModule
from .mvtec_base_model import DownsampleBlock
from .mvtec_base_model import ResidualBlock
from .mvtec_base_model import UpsampleBlock

CHANNELS = [32, 64, 128]


def init_conv_blocks(channel_in: int, channel_out: int, activation_fn: nn) -> nn.Module:
...
class MVTec_Encoder(BaseModule):
    ...
    def get_depths_info(self) -> [int, int]:
        """
        Returns
        ------
        self.last_depth : int
...
class MVTecNet_AutoEncoder(BaseModule):
    ...
    def __init__(self, input_shape: int, code_length: int, use_selectors: bool):
        ...
        self.encoder = MVTec_Encoder(
            input_shape=input_shape, code_length=code_length, idx_list_enc=[], use_selectors=use_selectors
        )
        ...
        self.decoder = MVTec_Decoder(
            code_length=code_length, deepest_shape=deepest_shape, last_depth=last_depth, output_shape=input_shape
        )
