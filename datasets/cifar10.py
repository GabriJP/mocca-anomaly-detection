...
class CIFAR10_DataHolder(object):
    ...
    def __init_train_test_datasets(self, normal_class: int) -> None:
        ...
        self.transform = transforms.Compose(
            [
                ...
                transforms.Normalize(
                    [min_max[normal_class][0]] * 3, [min_max[normal_class][1] - min_max[normal_class][0]] * 3
                ),
            ]
        )
        ...
        self.train_set = MyCIFAR10(
            root=self.root, train=True, download=True, transform=self.transform, target_transform=self.target_transform
        )
        ...
        self.test_set = MyCIFAR10(
            root=self.root, train=False, download=True, transform=self.transform, target_transform=self.target_transform
        )
    ...
    def get_loaders(
        ...
        test_loader = DataLoader(
            dataset=self.test_set, batch_size=batch_size, pin_memory=pin_memory, num_workers=num_workers
        )
