...
class MVTec_DataHolder:
    ...
    def __init__(
        self, data_path: str, category: str, image_size: int, patch_size: int, rotation_range: tuple, is_texture: bool
    ):
                ...
                if epoch == 0:
                    cache_np = [
                        ...
                        for x, _ in tqdm(
                            ...
                            desc=f"Caching epoch: {epoch + 1}/{nb_epochs + 1}",
                            leave=False,
                        )
                    ]
                else:
...
if __name__ == "__main__":
    ...
    for category in classes:
        if category in textures:
            args = dict(category=category, image_size=512, patch_size=64, rotation_range=(0, 45), texture=True)
        elif category in objects_1:
            args = dict(category=category, image_size=128, patch_size=-1, rotation_range=(-45, 45), texture=True)
        else:
            args = dict(category=category, image_size=128, patch_size=-1, rotation_range=(0, 0), texture=False)
