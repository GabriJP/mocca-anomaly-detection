...
from trainers.trainer_mvtec import pretrain
from trainers.trainer_mvtec import test
from trainers.trainer_mvtec import train
from utils import eval_spheres_centers
from utils import get_out_dir
from utils import load_mvtec_model_from_checkpoint
...
def test_models(
    ...
    table.add_row(
        [
            ...
            code_length,
            idx_list_enc,
            net_cehckpoint.split("/")[-2].split("-")[7].split("_")[-1]
            + "-"
            + net_cehckpoint.split("/")[-2].split("-")[8],
            normal_class,
            boundary,
    ...
    out_df = out_df.append(
        dict(
            ...
            weight_decay=net_cehckpoint.split("/")[-2].split("-")[7].split("_")[-1]
            + "-"
            + net_cehckpoint.split("/")[-2].split("-")[8],
...
def main(args):
    ...
    data_holder = DataManager(
        dataset_name="MVTec_Anomaly", data_path=args.data_path, normal_class=args.normal_class, only_test=args.test
    ).get_data_holder()
    ...
    logger.info("\n" f"\n\t\t\t\tNormal class  : {args.normal_class}" f"\n\t\t\t\tBatch size    : {args.batch_size}")
    if not only_test:
        logger.info(
            f"TRAIN:"
    ...
    if args.pretrain:
        ...
        pretrain_tb_writer = SummaryWriter(
            os.path.join(args.output_path, "mvtec", str(args.normal_class), "tb_runs/pretrain", tmp)
        )

        # Init AutoEncoder
        ae_net = MVTecNet_AutoEncoder(
            input_shape=input_shape, code_length=args.code_length, use_selectors=args.use_selectors
        )
    ...
    if args.train:
        ...
        train_tb_writer = SummaryWriter(
            os.path.join(args.output_path, "mvtec", str(args.normal_class), "tb_runs/train", tmp)
        )
    ...
    if args.test:
        ...
        if is_file:
            ...
            for model_ckp in tqdm(
                os.listdir(net_cehckpoint), total=len(os.listdir(net_cehckpoint)), desc="Running on models"
            ):
                out_df = test_models(
                    test_loader=test_loader,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-awd", "--ae-weight-decay", type=float, default=0.5e-6, help="Warm up learning rate (default: 0.5e-4)"
    )
